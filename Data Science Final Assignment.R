...
#all Disaster subgroup 

# Group disasters by period and subgroup
disasters_counts_by_period <- disaster_df %>%
  filter(between(`Start Year`, 1980, 2019)) %>%  # Filter years from 1980 to 2019
  mutate(Period = case_when(
    `Start Year` <= 1999 ~ "1980-1999",
    `Start Year` > 1999 ~ "2000-2019"
  )) %>%
  group_by(Period, `Disaster Subgroup`) %>%
  summarise(Total_Disasters = n(), .groups = 'drop')

# Print the results
print(disasters_counts_by_period)
